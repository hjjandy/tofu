...
public abstract class BaseEntryPointCreator implements IEntryPointCreator {
	...
	protected Map<Integer, Value> cachedParams = new HashMap<>();
	...
	protected Stmt buildMethodCall(SootMethod methodToCall, Body body, Local classLocal, LocalGenerator gen,
		...
		if (methodToCall.getParameterCount() > 0) {
			...
			for (Type tp : methodToCall.getParameterTypes()) {
				...
				if (!cachedParams.isEmpty() && cachedParams.containsKey(idxParam)) {
					Value cachedP = cachedParams.get(idxParam);
					args.add(cachedP);
				} else {
...
//				body.getUnits().add(Jimple.v().newAssignStmt((Value) val, NullConstant.v()));
	...
	private Value buildArrayOfType(Body body, LocalGenerator gen, ArrayType tp, Set<SootClass> constructionStack,
		...
		AssignStmt assign = Jimple.v().newAssignStmt(Jimple.v().newArrayRef(local, IntConstant.v(0)),
	 ...
	 * completed successfully, otherwise null.
	 */
	private Local generateClassConstructor(SootClass createdClass, Body body, Set<SootClass> constructionStack,
										   Set<SootClass> parentClasses) {
		...
		String outerClass = isInnerClass ? createdClass.getName()
													   .substring(0, createdClass.getName().lastIndexOf("$")) : "";

		// Make sure that we don't run into loops
		if (!constructionStack.add(createdClass)) {
			logger.warn(
					"Ran into a constructor generation loop for class " + createdClass + ", substituting with null...");
		...
		if (createdClass.isInterface() || createdClass.isAbstract()) {
			if (substituteCallParams) {
				...
				logger.warn(
						"Cannot create valid constructor for {}, because it is {} and cannot substitute with subclass",
						createdClass,
