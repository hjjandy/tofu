 ...
 */
public class TofuEntryPointCreator extends BaseEntryPointCreator implements IEntryPointCreator {
	...
	/**
	 * Creates a new instance of the {@link TofuEntryPointCreator} class
	 ...
	 */
	public TofuEntryPointCreator(Collection<String> androidClasses) {
	 ...
	 */
	public TofuEntryPointCreator(Collection<String> androidClasses, Collection<String> additionalEntryPoints) {
		this.androidClasses = androidClasses;
	...
	protected SootMethod createDummyMainInternal(SootMethod emptySootMethod) {
		...
		// the application object's onCreate() is called.
		{
			...
			for (String className : classMap.keySet()) {
				...
				if (getComponentType(currentClass) == ComponentType.ContentProvider) {
					...
					buildMethodCall(findMethod(currentClass, TofuEntryPointConstants.CONTENTPROVIDER_ONCREATE), body,
									localVal, generator);
		...
		for (Entry<String, Set<String>> entry : classMap.entrySet()) {
			...
			for (SootClass sc : extendedClasses)
				if (sc.getName().equals(TofuEntryPointConstants.APPLICATIONCLASS)) {
					...
					searchAndBuildMethod(TofuEntryPointConstants.APPLICATION_ONCREATE, applicationClass,
										 entry.getValue(), applicationLocal);
		...
		for (Entry<String, Set<String>> entry : classMap.entrySet()) {
			...
			try {
				ComponentType componentType = getComponentType(currentClass);if (currentClass.getName().endsWith("PullRefreshContainerView")) {System.out.println("AAAAAAAAAA " + componentType);}
				...
				boolean instanceNeeded =
						componentType == ComponentType.Activity || componentType == ComponentType.Service ||
						componentType == ComponentType.BroadcastReceiver ||
						componentType == ComponentType.ContentProvider || componentType == ComponentType.Fragment ||
						componentType == ComponentType.PagerAdapter ||
						componentType == ComponentType.ViewPagerOnPageChangeListener ||
						componentType == ComponentType.CustomView;
				...
				switch (componentType) {
					case Activity:
						generateActivityLifecycle(entry.getValue(), currentClass, endClassStmt, classLocal);
						break;
					case Service:
						generateServiceLifecycle(entry.getValue(), currentClass, endClassStmt, classLocal);
						break;
					case BroadcastReceiver:
						generateBroadcastReceiverLifecycle(entry.getValue(), currentClass, endClassStmt, classLocal);
						break;
					case ContentProvider:
						generateContentProviderLifecycle(entry.getValue(), currentClass, endClassStmt, classLocal);
						break;
					case Fragment:
						generateFragmentLifecycle(entry.getValue(), currentClass, endClassStmt, classLocal);
						break;
					case PagerAdapter:
						generatePagerAdapterLifecycle(entry.getValue(), currentClass, endClassStmt, classLocal);
						break;
					case ViewPagerOnPageChangeListener:
						generateViewPagerOnPageChangeListenerLifecycle(entry.getValue(), currentClass, endClassStmt,
																	   classLocal);
						break;
					case CustomView:
						generateCustomViewLifecycle(entry.getValue(), currentClass, endClassStmt, classLocal);
						break;
					case Plain:
		...
		if (applicationLocal != null)
			searchAndBuildMethod(TofuEntryPointConstants.APPLICATION_ONTERMINATE, applicationClass,
								 classMap.get(applicationClass.getName()), applicationLocal);
	...
	private List<SootClass> collectImplementedInterfaces(SootClass sc, List<SootClass> extendedClasses) {
		List<SootClass> ret = new LinkedList<>(extendedClasses);
	...
	private ComponentType getComponentType(SootClass currentClass) {
		...
		if (TofuResParser.isCustomView(currentClass.getName())) {
			ctype = ComponentType.CustomView;
		} else {
			List<SootClass> extendedClasses = Scene.v().getActiveHierarchy().getSuperclassesOf(currentClass);
			extendedClasses = collectImplementedInterfaces(currentClass, extendedClasses);
			for (SootClass sc : extendedClasses) {
				if (sc.getName().equals(TofuEntryPointConstants.APPLICATIONCLASS))
					ctype = ComponentType.Application;
				else if (sc.getName().equals(TofuEntryPointConstants.ACTIVITYCLASS))
					ctype = ComponentType.Activity;
				else if (sc.getName().equals(TofuEntryPointConstants.SERVICECLASS))
					ctype = ComponentType.Service;
				else if (sc.getName().equals(TofuEntryPointConstants.BROADCASTRECEIVERCLASS))
					ctype = ComponentType.BroadcastReceiver;
				else if (sc.getName().equals(TofuEntryPointConstants.CONTENTPROVIDERCLASS))
					ctype = ComponentType.ContentProvider;
				else if (sc.getName().equals(TofuEntryPointConstants.V4FRAGMENTCLASS)) {
					ctype = ComponentType.Fragment;
				} else if (sc.getName().equals(TofuEntryPointConstants.FRAGMENTCLASS)) {
					ctype = ComponentType.Fragment;
				} else if (sc.getName().equals(TofuEntryPointConstants.V4PagerAdapter)) {
					ctype = ComponentType.PagerAdapter;
				} else if (sc.getName().equals(TofuEntryPointConstants.V4ViewPagerOnPageChangeListener)) {
					ctype = ComponentType.ViewPagerOnPageChangeListener;
				} else
					continue;
	...
	private void generateContentProviderLifecycle(Set<String> entryPoints, SootClass currentClass, NopStmt endClassStmt,
		...
		// Stmt onCreateStmt = searchAndBuildMethod(TofuEntryPointConstants.CONTENTPROVIDER_ONCREATE, currentClass, entryPoints, classLocal);
	...
	private void generateBroadcastReceiverLifecycle(Set<String> entryPoints, SootClass currentClass,
		...
		Stmt onReceiveStmt = searchAndBuildMethod(TofuEntryPointConstants.BROADCAST_ONRECEIVE, currentClass,
												  entryPoints, classLocal);
		//methods
		NopStmt startWhileStmt = Jimple.v().newNopStmt();
		NopStmt endWhileStmt = Jimple.v().newNopStmt();
		body.getUnits().add(startWhileStmt);
	...
	private void generateServiceLifecycle(Set<String> entryPoints, SootClass currentClass, NopStmt endClassStmt,
		...
		// 1. onCreate:
		searchAndBuildMethod(TofuEntryPointConstants.SERVICE_ONCREATE, currentClass, entryPoints, classLocal);

		//service has two different lifecycles:
		//lifecycle1:
		//2. onStart:
		searchAndBuildMethod(TofuEntryPointConstants.SERVICE_ONSTART1, currentClass, entryPoints, classLocal);
		...
		searchAndBuildMethod(TofuEntryPointConstants.SERVICE_ONSTART2, currentClass, entryPoints, classLocal);
		...
		if (isGCMBaseIntentService) {
			for (String sig : TofuEntryPointConstants.getGCMIntentServiceMethods()) {
				SootMethod sm = findMethod(currentClass, sig);
				if (sm != null &&
					!sm.getDeclaringClass().getName().equals(TofuEntryPointConstants.GCMBASEINTENTSERVICECLASS))
					hasAdditionalMethods |= createPlainMethodCall(classLocal, sm);
			}
		} else if (isGCMListenerService) {
			for (String sig : TofuEntryPointConstants.getGCMListenerServiceMethods()) {
				SootMethod sm = findMethod(currentClass, sig);
				if (sm != null &&
					!sm.getDeclaringClass().getName().equals(TofuEntryPointConstants.GCMLISTENERSERVICECLASS))
					hasAdditionalMethods |= createPlainMethodCall(classLocal, sm);
			}
		}
		addCallbackMethods(currentClass);
		...
		//lifecycle2 start
		//onBind:
		searchAndBuildMethod(TofuEntryPointConstants.SERVICE_ONBIND, currentClass, entryPoints, classLocal);
		...
		if (isGCMBaseIntentService)
			for (String sig : TofuEntryPointConstants.getGCMIntentServiceMethods()) {
				SootMethod sm = findMethod(currentClass, sig);
				if (sm != null && !sm.getName().equals(TofuEntryPointConstants.GCMBASEINTENTSERVICECLASS))
					hasAdditionalMethods |= createPlainMethodCall(classLocal, sm);
			}
		addCallbackMethods(currentClass);
		...
		searchAndBuildMethod(TofuEntryPointConstants.SERVICE_ONUNBIND, currentClass, entryPoints, classLocal);
		createIfStmt(onDestroyStmt);    // fall through to rebind or go to destroy

		//onRebind:
		searchAndBuildMethod(TofuEntryPointConstants.SERVICE_ONREBIND, currentClass, entryPoints, classLocal);
		...
		searchAndBuildMethod(TofuEntryPointConstants.SERVICE_ONDESTROY, currentClass, entryPoints, classLocal);
	...
	private boolean createPlainMethodCall(Local classLocal, SootMethod currentMethod) {
		...
		if (TofuEntryPointConstants.getServiceLifecycleMethods().contains(currentMethod.getSubSignature()))
			return false;
	...
	private boolean isGCMBaseIntentService(SootClass currentClass) {
		while (currentClass.hasSuperclass()) {
			if (currentClass.getSuperclass().getName().equals(TofuEntryPointConstants.GCMBASEINTENTSERVICECLASS))
				return true;
	...
	private boolean isGCMListenerService(SootClass currentClass) {
		while (currentClass.hasSuperclass()) {
			if (currentClass.getSuperclass().getName().equals(TofuEntryPointConstants.GCMLISTENERSERVICECLASS))
				return true;
			currentClass = currentClass.getSuperclass();
		}
		return false;
	...
	private void generateActivityLifecycle(Set<String> entryPoints, SootClass currentClass, NopStmt endClassStmt,
		...
		{
			Stmt onCreateStmt2 = searchAndBuildMethod(TofuEntryPointConstants.ACTIVITY_ONCREATE, currentClass,
													  entryPoints, classLocal);
			boolean found = addCallbackMethods(applicationClass, referenceClasses,
											   TofuEntryPointConstants.APPLIFECYCLECALLBACK_ONACTIVITYCREATED);
			if (found && onCreateStmt2 != null)
				createIfStmt(onCreateStmt2);
			//System.out.println("onCreate: " + onCreateStmt2);
		}
		...
		{
			Stmt onStartStmt2 = searchAndBuildMethod(TofuEntryPointConstants.ACTIVITY_ONSTART, currentClass,
													 entryPoints, classLocal);
			boolean found = addCallbackMethods(applicationClass, referenceClasses,
											   TofuEntryPointConstants.APPLIFECYCLECALLBACK_ONACTIVITYSTARTED);
		...
		// state has previously been stored.
		{
			...
			searchAndBuildMethod(TofuEntryPointConstants.ACTIVITY_ONRESTOREINSTANCESTATE, currentClass, entryPoints,
								 classLocal);
			body.getUnits().add(afterOnRestore);
		}
		searchAndBuildMethod(TofuEntryPointConstants.ACTIVITY_ONPOSTCREATE, currentClass, entryPoints, classLocal);

		//3. onResume:
		Stmt onResumeStmt = Jimple.v().newNopStmt();
		body.getUnits().add(onResumeStmt);
		{
			Stmt onResumeStmt2 = searchAndBuildMethod(TofuEntryPointConstants.ACTIVITY_ONRESUME, currentClass,
													  entryPoints, classLocal);
			boolean found = addCallbackMethods(applicationClass, referenceClasses,
											   TofuEntryPointConstants.APPLIFECYCLECALLBACK_ONACTIVITYRESUMED);
			if (found && onResumeStmt2 != null)
				createIfStmt(onResumeStmt2);
		}
		searchAndBuildMethod(TofuEntryPointConstants.ACTIVITY_ONPOSTRESUME, currentClass, entryPoints, classLocal);
		...
		if (modelAdditionalMethods)
			for (SootMethod currentMethod : currentClass.getMethods())
				if (entryPoints.contains(currentMethod.toString()) &&
					!TofuEntryPointConstants.getActivityLifecycleMethods().contains(currentMethod.getSubSignature()))
		...
		//4. onPause:
		Stmt onPause = searchAndBuildMethod(TofuEntryPointConstants.ACTIVITY_ONPAUSE, currentClass, entryPoints,
											classLocal);
		boolean hasAppOnPause = addCallbackMethods(applicationClass, referenceClasses,
												   TofuEntryPointConstants.APPLIFECYCLECALLBACK_ONACTIVITYPAUSED);
		if (hasAppOnPause && onPause != null)
			createIfStmt(onPause);
		searchAndBuildMethod(TofuEntryPointConstants.ACTIVITY_ONCREATEDESCRIPTION, currentClass, entryPoints,
							 classLocal);
		Stmt onSaveInstance = searchAndBuildMethod(TofuEntryPointConstants.ACTIVITY_ONSAVEINSTANCESTATE, currentClass,
												   entryPoints, classLocal);
		boolean hasAppOnSaveInstance = addCallbackMethods(applicationClass, referenceClasses,
														  TofuEntryPointConstants.APPLIFECYCLECALLBACK_ONACTIVITYSAVEINSTANCESTATE);
		...
		//5. onStop:
		Stmt onStop = searchAndBuildMethod(TofuEntryPointConstants.ACTIVITY_ONSTOP, currentClass, entryPoints,
										   classLocal);
		boolean hasAppOnStop = addCallbackMethods(applicationClass, referenceClasses,
												  TofuEntryPointConstants.APPLIFECYCLECALLBACK_ONACTIVITYSTOPPED);
		...
		//6. onRestart:
		searchAndBuildMethod(TofuEntryPointConstants.ACTIVITY_ONRESTART, currentClass, entryPoints, classLocal);
		createIfStmt(onStartStmt);    // jump to onStart(), fall through to onDestroy()

		//7. onDestroy
		body.getUnits().add(stopToDestroyStmt);
		Stmt onDestroy = searchAndBuildMethod(TofuEntryPointConstants.ACTIVITY_ONDESTROY, currentClass, entryPoints,
											  classLocal);
		boolean hasAppOnDestroy = addCallbackMethods(applicationClass, referenceClasses,
													 TofuEntryPointConstants.APPLIFECYCLECALLBACK_ONACTIVITYDESTROYED);
	...
	private void generateFragmentLifecycle(Set<String> entryPoints, SootClass currentClass, NopStmt endClassStmt,
		...
		// 0.onAttach:
		Stmt onAttachStmt = Jimple.v().newNopStmt();
		body.getUnits().add(onAttachStmt);
		{
			Stmt onAttachStmt1 = searchAndBuildMethod(TofuEntryPointConstants.V4FRAGMENT_ONATTACH_Activity,
													  currentClass, entryPoints, classLocal);
			Stmt onAttachStmt2 = searchAndBuildMethod(TofuEntryPointConstants.V4FRAGMENT_ONATTACH_Context, currentClass,
													  entryPoints, classLocal);
			if (onAttachStmt1 != null && onAttachStmt2 != null) {
				Stmt fStartStmt = Jimple.v().newNopStmt();
				Stmt fEndStmt = Jimple.v().newNopStmt();
				JEqExpr cond = new JEqExpr(intCounter, IntConstant.v(conditionCounter++));
				JIfStmt ifStmt = new JIfStmt(cond, fStartStmt);
				body.getUnits().insertAfter(ifStmt, onAttachStmt);
				Stmt tGotoStmt = Jimple.v().newGotoStmt(fEndStmt);
				body.getUnits().insertAfter(tGotoStmt, onAttachStmt1);
				body.getUnits().insertAfter(fStartStmt, tGotoStmt);
				body.getUnits().insertAfter(fEndStmt, onAttachStmt2);
			}
		}
		...
		body.getUnits().add(onCreateStmt);
		{
			Stmt onCreateStmt2 = searchAndBuildMethod(TofuEntryPointConstants.V4FRAGMENT_ONCREATE, currentClass,
													  entryPoints, classLocal);
		}

		// 2. onCreateView:
		Value retOfOnCreateView = null;
		Stmt onCreateViewStmt = Jimple.v().newNopStmt();
		body.getUnits().add(onCreateViewStmt);
		{
			Stmt onCreateView2 = searchAndBuildMethod(TofuEntryPointConstants.V4FRAGMENT_ONCREATEVIEW, currentClass,
													  entryPoints, classLocal);
			if (onCreateView2 != null && onCreateView2 instanceof AssignStmt) {
				retOfOnCreateView = ((AssignStmt) onCreateView2).getLeftOp();
			}
		}

		// 3. onViewCreated:
		Stmt onViewCreatedStmt = Jimple.v().newNopStmt();
		body.getUnits().add(onViewCreatedStmt);
		{
			if (retOfOnCreateView != null) {
				cachedParams.put(0, retOfOnCreateView);
			}
			Stmt onViewCreated2 = searchAndBuildMethod(TofuEntryPointConstants.V4FRAGMENT_ONVIEWCREATED, currentClass,
													   entryPoints, classLocal);
			cachedParams.clear();
		}

		// 4. onActivityCreated:
		Stmt onActivityCreatedStmt = Jimple.v().newNopStmt();
		body.getUnits().add(onActivityCreatedStmt);
		{
			Stmt onActivityCreated2 = searchAndBuildMethod(TofuEntryPointConstants.V4FRAGMENT_ONACTIVITYCREATED,
														   currentClass, entryPoints, classLocal);
		}
		...
		body.getUnits().add(onStartStmt);
		{
			Stmt onStartStmt2 = searchAndBuildMethod(TofuEntryPointConstants.V4FRAGMENT_ONSTART, currentClass,
													 entryPoints, classLocal);
		}
		...
		body.getUnits().add(onResumeStmt);
		{
			Stmt onResumeStmt2 = searchAndBuildMethod(TofuEntryPointConstants.V4FRAGMENT_ONRESUME, currentClass,
													  entryPoints, classLocal);
		}

		// 7. onPause:
		Stmt onPause = searchAndBuildMethod(TofuEntryPointConstants.V4FRAGMENT_ONPAUSE, currentClass, entryPoints,
											classLocal);

		// 8. onStop:
		Stmt onStop = searchAndBuildMethod(TofuEntryPointConstants.V4FRAGMENT_ONSTOP, currentClass, entryPoints,
										   classLocal);

		// 9. onDestroyView:
		searchAndBuildMethod(TofuEntryPointConstants.V4FRAGMENT_ONDESTROYVIEW, currentClass, entryPoints, classLocal);
		createIfStmt(onCreateViewStmt);    // jump to onStart(), fall through to onDestroy()

		// 10. onDestroy:
		Stmt onDestroy = searchAndBuildMethod(TofuEntryPointConstants.V4FRAGMENT_ONDESTROY, currentClass, entryPoints,
											  classLocal);

		// 11. onDetach:
		Stmt onDetach = searchAndBuildMethod(TofuEntryPointConstants.V4FRAGMENT_ONDETACH, currentClass, entryPoints,
											 classLocal);

		createIfStmt(endClassStmt);
	}

	private void generatePagerAdapterLifecycle(Set<String> entryPoints, SootClass currentClass, NopStmt endClassStmt,
											   Local classLocal) {
		// As we don't know the order in which the different Android lifecycles
		// run, we allow for each single one of them to be skipped
		createIfStmt(endClassStmt);
		...
		// 1.instantiateItem:
		Stmt instantiateItemStmt = Jimple.v().newNopStmt();
		body.getUnits().add(instantiateItemStmt);
		Value instantiatedObject = null;
		Value itemContaner = null;
		Value itemPosition = null;
		{
			Stmt initItemStmt = searchAndBuildMethod(TofuEntryPointConstants.V4PagerAdapter_INSTANTIATEITEM_ViewGroup,
													 currentClass, entryPoints, classLocal);
			if (initItemStmt != null) {
				itemContaner = initItemStmt.getInvokeExpr().getArg(0);
				itemPosition = initItemStmt.getInvokeExpr().getArg(1);
				if (initItemStmt instanceof AssignStmt) {
					instantiatedObject = ((AssignStmt) initItemStmt).getLeftOp();
				}
			}
		}
		...
		// 2. destroyItem:
		createIfStmt(endClassStmt);
		Stmt destroyItemStmt = Jimple.v().newNopStmt();
		body.getUnits().add(destroyItemStmt);
		{
			if (itemContaner != null) {
				cachedParams.put(0, itemContaner);
			}
			if (itemPosition != null) {
				cachedParams.put(1, itemPosition);
			}
			if (instantiatedObject != null) {
				cachedParams.put(2, instantiatedObject);
			}
			Stmt destroyStmt = searchAndBuildMethod(TofuEntryPointConstants.V4PagerAdapter_DESTROYITEM_ViewGroup,
													currentClass, entryPoints, classLocal);
			cachedParams.clear();
		}
	...
	private void generateViewPagerOnPageChangeListenerLifecycle(Set<String> entryPoints, SootClass currentClass,
																NopStmt endClassStmt, Local classLocal) {
		// As we don't know the order in which the different Android lifecycles
		// run, we allow for each single one of them to be skipped
		createIfStmt(endClassStmt);
		...
		Stmt opsscStmt = Jimple.v().newNopStmt();
		Stmt opsStmt = Jimple.v().newNopStmt();
		Stmt opselStmt = Jimple.v().newNopStmt();
		createIfStmt(opsStmt);// --> scrolled
		createIfStmt(opselStmt);// --> selected

		// 1.onPageScrollStateChanged:
		body.getUnits().add(opsscStmt);
		{
			Stmt stateChangedStmt = searchAndBuildMethod(
					TofuEntryPointConstants.V4ViewPagerOnPageChangeListener_onPageScrollStateChanged, currentClass,
					entryPoints, classLocal);
			createIfStmt(opselStmt);// statechaned --> selected
		}

		// 2. onPageScrolled:
		createIfStmt(endClassStmt);
		body.getUnits().add(opsStmt);
		{
			Stmt scrolledStmt = searchAndBuildMethod(
					TofuEntryPointConstants.V4ViewPagerOnPageChangeListener_onPageScrolled, currentClass, entryPoints,
					classLocal);
			createIfStmt(opsscStmt); // scrolled --> statechanged
		}

		// 3. onPageSelected:
		createIfStmt(endClassStmt);
		body.getUnits().add(opselStmt);
		{
			Stmt selectedStmt = searchAndBuildMethod(
					TofuEntryPointConstants.V4ViewPagerOnPageChangeListener_onPageSelected, currentClass, entryPoints,
					classLocal);
			createIfStmt(opsscStmt);// selected --> statechaned
			createIfStmt(opsStmt);//selected --> scrolled
		}

		createIfStmt(endClassStmt);
	}

	private void generateCustomViewLifecycle(Set<String> entryPoints, SootClass currentClass, NopStmt endClassStmt,
											 Local classLocal) {
		// As we don't know the order in which the different Android lifecycles
		// run, we allow for each single one of them to be skipped
		createIfStmt(endClassStmt);
		...
		// 1.onMeasure:
		Stmt omStmt = Jimple.v().newNopStmt();
		body.getUnits().add(omStmt);
		{
			Stmt onMeasureStmt = searchAndBuildMethod(TofuEntryPointConstants.CustomView_onMeasure, currentClass,
													  entryPoints, classLocal);
		}

		// 2. onLayout:
		Stmt olStmt = Jimple.v().newNopStmt();
		body.getUnits().add(olStmt);
		{
			Stmt onLayoutStmt = searchAndBuildMethod(TofuEntryPointConstants.CustomView_onLayout, currentClass,
													 entryPoints, classLocal);
		}

		// 3. onDraw:
		Stmt odStmt = Jimple.v().newNopStmt();
		body.getUnits().add(odStmt);
		{
			Stmt onDrawStmt = searchAndBuildMethod(TofuEntryPointConstants.CustomView_onDraw, currentClass, entryPoints,
												   classLocal);
		}
	...
	private void addApplicationCallbackMethods() {
		...
		for (String methodSig : this.callbackFunctions.get(applicationClass.getName())) {
			...
			if (TofuEntryPointConstants.getApplicationLifecycleMethods().contains(methodAndClass.getSubSignature()))
				continue;
...
//				logger.warn("Could not find callback method {}", methodAndClass.getSignature());
			...
			if (getComponentType(theClass) == ComponentType.Activity &&
				TofuEntryPointConstants.getActivityLifecycleMethods().contains(theMethod.getSubSignature()))
				continue;
			if (getComponentType(theClass) == ComponentType.Service &&
				TofuEntryPointConstants.getServiceLifecycleMethods().contains(theMethod.getSubSignature()))
				continue;
			if (getComponentType(theClass) == ComponentType.BroadcastReceiver &&
				TofuEntryPointConstants.getBroadcastLifecycleMethods().contains(theMethod.getSubSignature()))
				continue;
			if (getComponentType(theClass) == ComponentType.ContentProvider &&
				TofuEntryPointConstants.getContentproviderLifecycleMethods().contains(theMethod.getSubSignature()))
				continue;
	...
	private Stmt searchAndBuildMethod(String subsignature, SootClass currentClass, Set<String> entryPoints,
		...
		if (TofuEntryPointConstants.isLifecycleClass(method.getDeclaringClass().getName()))
			return null;
