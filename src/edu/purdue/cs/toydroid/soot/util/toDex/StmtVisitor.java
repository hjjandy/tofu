...
import edu.purdue.cs.toydroid.soot.util.toDex.instructions.*;
...
class StmtVisitor implements StmtSwitch {
	...
	public void caseIdentityStmt(IdentityStmt stmt) {
		...
		if (rhs instanceof CaughtExceptionRef) {
            ...
            this.insnRegisterMap.put(insns.get(insns.size() - 1), LocalRegisterAssignmentInformation
					.v(localReg, (Local)lhs));
			...
			parameterInstructionsList.add(
					LocalRegisterAssignmentInformation.v(regAlloc.asLocal(localForThis).clone(), localForThis));
		} else {
