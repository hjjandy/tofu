...
import edu.purdue.cs.toydroid.soot.util.toDex.instructions.Insn;
import edu.purdue.cs.toydroid.soot.util.toDex.instructions.Insn10t;
import edu.purdue.cs.toydroid.soot.util.toDex.instructions.Insn30t;
import edu.purdue.cs.toydroid.soot.util.toDex.instructions.InsnWithOffset;
...
public class DexPrinter {
	...
	private Map<String, Set<SootClass>> dex2ReachableClasses;

	private File originalApk;

	public DexPrinter() {
		apiVersion = Scene.v().getAndroidAPIVersion();
		dex2ReachableClasses = new HashMap<>();
	}
	...
	private static boolean isSignatureFile(String fileName) {
		...
		sigFileRegex.append("(\\.SF|\\.DSA|\\.RSA|\\.EC|\\.MF)$");
	...
	public void partitionReachableClasses(Set<String> dexOfModified) {
		for (String mdex : dexOfModified) {
			Set<String> dClasses = TofuDexClasses.getClasses(mdex);
			if (dClasses == null) {
				continue;
			}
			for (String clazz : dClasses) {
				SootClass klass = Scene.v().getSootClass(clazz);
				Set<SootClass> dexClasses = dex2ReachableClasses.get(mdex);
				if (dexClasses == null) {
					dexClasses = new HashSet<>();
					dex2ReachableClasses.put(mdex, dexClasses);
				}
				dexClasses.add(klass);
			}
		}
	}

	private void newDexBuilder() {
		dexFile = new DexBuilder(Opcodes.forApi(apiVersion));
	}
	...
	private void printApk(String outputDir, File originalApk) throws IOException {
		...
		Set<String> dexNames = dex2ReachableClasses.keySet();
		for (String dex : dexNames) {
			G.v().out.println("Start writing: " + dex);
			newDexBuilder();
			Set<SootClass> classesInDex = dex2ReachableClasses.get(dex);
			for (SootClass sc : classesInDex) {
				add(sc);
			}
			File tmpFile = File.createTempFile(dex + ".toDex", null);
			FileInputStream fis = new FileInputStream(tmpFile);
			try {
				outputApk.putNextEntry(new ZipEntry(dex));
			...
			G.v().out.println("Done writing: " + dex);
		}
		if (outputApk != null)
			outputApk.close();

		G.v().out.println("Writing APK to: " + outAPK);
...
//				continue;
//			}
			if (dex2ReachableClasses.containsKey(sourceEntryName) || isSignatureFile(sourceEntryName)) {
				continue;
			}
			...
			zipEntryInput.close();
			G.v().out.println("Done writing: " + sourceEntryName);
		}
	}
	...
	private void addAsClassDefItem(SootClass c) {
		...
		if (!c.getFields().isEmpty()) {
			...
			for (SootField f : c.getFields()) {
				...
				BuilderField field = dexFile
						.internField(classType, f.getName(), SootToDexUtils.getDexTypeDescriptor(f.getType()),
																									 ...
																									 SootToDexUtils
																											 .getDexClassName(
																													 c.getOuterClass()
																													  .getName())));
				annotations
						.add(new ImmutableAnnotation(AnnotationVisibility.SYSTEM, "Ldalvik/annotation/EnclosingClass;",
				...
				for (Type tp : sm.getParameterTypes()) {
					...
					parameters
							.add(new ImmutableMethodParameter(paramType, buildMethodParameterAnnotations(sm, paramIdx),
															  sm.isConcrete() && parameterNames != null ? parameterNames
																	  .get(paramIdx) : null));
					paramIdx++;
				}
			}
	...
	private MethodImplementation toMethodImplementation(SootMethod m) {
		...
		FastDexTrapTightener.v().transform(activeBody);//HJJ
		...
		//TrapSplitter.v().transform(activeBody);//HJJ
...
//		originalApk = sourceForClass;
	...
	public void setOriginalApk(String ApkFile) {
		originalApk = new File(ApkFile);
	}
