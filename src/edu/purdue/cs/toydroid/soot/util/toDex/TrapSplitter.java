...
public class TrapSplitter extends BodyTransformer {
	private static TrapSplitter instance_soot_toDex_TrapSplitter;
	...
	public static TrapSplitter v() {
		if (instance_soot_toDex_TrapSplitter == null) {
			instance_soot_toDex_TrapSplitter = new TrapSplitter();
	...
	private TrapOverlap getNextOverlap(Body b) {
		for (Trap t1 : b.getTraps()) {
			...
			for (Unit splitUnit = t1.getBeginUnit();
				 splitUnit != t1.getEndUnit(); splitUnit = b.getUnits().getSuccOf(splitUnit)) {
